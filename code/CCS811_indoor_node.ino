...
//上传
#define BC26_ALI_SENSOR_UPLOAD "AT+QMTPUB=0,12,1,0,\"/sys/%s/%s/thing/event/property/post\",\"{'id':'110','version':'1.0','method':'thing.event.property.post','params':{'CO2':%d,'co2Threshold':%d,\"alarmState\":%d}}\"\r\n"

//下发
#define BC26_ALI_SUB_SET_t "AT+QMTSUB=1,12,\"/sys/%s/%s/thing/service/thresholdChange\",0 \r\n"

// #define AT_QMT_SUB_SET "AT+QMTSUB=0,2,\"/sys/%s/%s/thing/service/alarmFree\",1 \r\n"
// #define AT_QMT_SUB_SET_1 "AT+QMTSUB=0,2,\"/sys/%s/%s/thing/service/alarmOpen\",1 \r\n"

#define ALARM_FREE "alarmFree"
#define ALARM_OPEN "alarmOpen"

//三元组
#define ProductKey "gh7xuFml7nN"
#define DeviceName "Arduino_CCS811"
#define DeviceSecret "e2a7e00b20a6e9877f668eb76cd17cf1"
...
int co2Threshold;
bool status;
const char *value_TH = "co2Threshold";
...
bool BC26_sensor_upload(int CO2PPM, int co2Threshold, bool status);
...
void loop()
{
  while (1)
  {
    ...
    BC26_sensor_upload(CO2PPM, co2Threshold, status);
...
//  if (status == true) //超过阈值
//  {
//    if (flag_alarmFree == true)//接收到解除警报
//    {
//      alarm_run = false;
//      flag_alarmFree = false;//仅关闭这一次，接受后复原flag_alarmFree
...
bool BC26_sensor_upload(int CO2PPM, int co2Threshold, bool status)
{
  ...
  snprintf(ATcmd, BUF_LEN1, BC26_ALI_SENSOR_UPLOAD, ProductKey, DeviceName, CO2PPM, co2Threshold, status);
...
bool BC26_sensor_download()
{
  if (check_send_cmd("AT", "version", 1))
  {
    ...
    Serial.println("Refreshed Local co2Threshold：");
